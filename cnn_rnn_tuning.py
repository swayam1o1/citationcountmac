...
Hyperparameter tuning for CNN and vanilla RNN regressors predicting 2022 citations.

Features (ONLY)
- cites_2021
- cosine_tfidf_similarity

We treat the 2 features as a length-2 sequence with 1 channel.

Outputs
- cnn/<exp_id>/config_and_metrics.json
- cnn/<exp_id>/test_predictions.csv
- rnn/<exp_id>/config_and_metrics.json
- rnn/<exp_id>/test_predictions.csv

NOTE: Your prompt requested (ann, mlp, lstm, cnn) as the four output folders,
but also requested RNN. This script writes RNN outputs to `rnn/`.
If you want RNN outputs under `mlp/` or `cnn/` instead, say so and I’ll adjust.
"""
...
class RNNRegressor(nn.Module):
    """Vanilla RNN treating two features as timesteps."""
...
def main() -> None:
    ...
    OUT_CNN.mkdir(exist_ok=True)
    OUT_RNN.mkdir(exist_ok=True)
    ...
    # Hold model-specific knobs constant to keep cardinality at 7,500.
    cnn_channels = 16
    cnn_kernel_size = 3
    cnn_dropout = 0.2
                    ...
                    for opt in common_opts:
                        for act in activation_grid:
                            cfg = {
                                ...
                                "target": "cites_2022",
                                "seed": SEED,
                                "sequence_representation": "[cites_2021, cosine_tfidf_similarity] as 2 steps",
                            }
                            exp_name = _stable_exp_id(cfg)
                            if SKIP_EXISTING and (OUT_CNN / exp_name).exists():
                                continue
                ...
                for lr in common_lrs:
                    for opt in common_opts:
                        for act in activation_grid:
                            ...
                            exp_name = _stable_exp_id(cfg)
                            if SKIP_EXISTING and (OUT_RNN / exp_name).exists():
                                continue
    ...
    print(f"Done. New experiments written: CNN={cnn_written} | RNN={rnn_written}")
