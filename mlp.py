...
Full 7,500-combo grid search for an MLP predicting 2022 citations, matching the other tuning scripts.

Outputs
- mlp/<exp_id>/config_and_metrics.json
- mlp/<exp_id>/test_predictions.csv
"""
...
class MLPRegressor(nn.Module):
    def __init__(self, activation: str, dropout: float = DROPOUT):
        super().__init__()
...
def make_split_with_ids(df: pd.DataFrame, train_ratio: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, List[str]]:
    n = len(df)
    idx = np.arange(n)
...
def main() -> None:
                ...
                for lr in COMMON_LRS:
                    for opt in COMMON_OPTS:
                        for act in ACTIVATION_GRID:
                            ...
                            if SKIP_EXISTING and (OUT_MLP / exp_name).exists():
                                continue

                            model = MLPRegressor(activation=str(act))
    ...
    print(f"Done. New MLP experiments written: {mlp_written}")


if __name__ == "__main__":
    main()"""mlp_hyperparam_search_2022_citations.py
...
=======
    1) 2021 citation count (from /Users/swayamprabha/Downloads/1992.xlsx)
